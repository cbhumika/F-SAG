...
# [[character begin offset, character end offset], word index, word, lemma, pos tag]
    ...
    #print(source)
    #print(target)
    sourceWords = [item[0] for item in source]
    targetWords = [item[0] for item in target]

    sourceLemmas = [item[0] for item in source]
    targetLemmas = [item[0] for item in target]

    sourcePosTags = [item[0] for item in source]
    targetPosTags = [item[0] for item in target]
    ...
    #print(sourceWordIndices)
    # construct the two matrices in the following loop
    for i in sourceWordIndices:
        #print(i)
        print(sourcePosTags[i-1][0])
        if i in (sourcePosTags[i-1][0].lower() != 'n' and sourcePosTags[i-1].lower() != 'prp'):
            continue

        numberOfNounsInSource += 1

        for j in targetWordIndices:
            if j in (targetPosTags[j-1][0].lower() != 'n' and targetPosTags[j-1].lower() != 'prp'):
                continue
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordParents:
                    if ((ktem[0], ltem[0]) in nounAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=ppdbSim) and (
                        (ktem[2]==ltem[2]) or
    ...
    for n in range(numberOfNounsInSource):
        ...
        for i in sourceWordIndices:
            if i in sourcePosTags[i-1][0].lower() != 'n' or sourceLemmas[i-1] in stop_words:
                continue

            for j in targetWordIndices:
                if j in targetPosTags[j-1][0].lower() != 'n' or targetLemmas[j-1] in stop_words:
                    continue
...
##############################################################################################################################
def alignMainVerbs(source, target):
# source and target:: each is a list of elements of the form:
# [[character begin offset, character end offset], word index, word, lemma, pos tag]
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            for ktem in sourceWordChildren:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in mainVerbAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=ppdbSim) and (
                            (ktem[2]==ltem[2]) or
...
##############################################################################################################################
def alignAdjectives(source, target):
# source and target:: each is a list of elements of the form:
# [[character begin offset, character end offset], word index, word, lemma, pos tag]
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordParents:
                    if ((ktem[0], ltem[0]) in adjectiveAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=ppdbSim) and ((ktem[2]==ltem[2]) or (ktem[2] in groupOfSimilarRelationsForNounParent and ltem[2] in groupOfSimilarRelationsForNounParent)):
            ...
            for ktem in sourceWordChildren:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in adjectiveAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=ppdbSim) and (ktem[2]==ltem[2]):
                        if (i, j) in evidenceCountsMatrix:
                            evidenceCountsMatrix[(i, j)] += max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
                        else:
                            evidenceCountsMatrix[(i, j)] = max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in adjectiveAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=ppdbSim) and (
                            (ktem[2]==ltem[2]) or
...
##############################################################################################################################
def alignAdverbs(source, target):
# source and target:: each is a list of elements of the form:
# [[character begin offset, character end offset], word index, word, lemma, pos tag]
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordParents:
                    if ((ktem[0], ltem[0]) in adverbAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=ppdbSim) and (ktem[2]==ltem[2]):
                        if (i, j) in evidenceCountsMatrix:
                            evidenceCountsMatrix[(i, j)] += max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
                        else:
                            evidenceCountsMatrix[(i, j)] = max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in adverbAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=ppdbSim) and (
                            (ktem[2]==ltem[2]) or
...
##############################################################################################################################
def alignNamedEntities(source, target):
# source and target:: each is a list of elements of the form:
# [[character begin offset, character end offset], word index, word, lemma, pos tag]
    ...
    for item in source:
        ...
        for jtem in sourceNamedEntities:
            if item[1] in jtem[1]:
                ...
                break
        if alreadyIncluded or (len(item[0]) >0 and not item[0][0].isupper()):
            continue
...
# ]
    ...
    #so = [item[0] for item in source]
    #ta = [item[0] for item in target]
    ...
    for i in targetWordIndices:
        if i in targetWordIndicesAlreadyAligned:
            continue
        if '-' in target[i-1][0] and target[i-1][0] != '-':
    ...
    neAlignments = alignNamedEntities(source, target)
    ...
    mainVerbAlignments = alignMainVerbs(source, target)
    ...
    nounAlignments = alignNouns(source, target)
    ...
    adjectiveAlignments = alignAdjectives(source, target)
    ...
    adverbAlignments = alignAdverbs(source, target)
...
def align(sentence1, sentence2):
    ...
    sentence1Lemmatized = sentence1
    sentence2Lemmatized = sentence2
